...
8. [Firebase Questions](#firebase-questions)
...
**Note:** Testing infrastructure is not yet implemented in this template. Below is the recommended pattern for when you add testing:
...
class PostsViewModelTest {
    ...
    @Test
    fun `loadPosts updates state with data`() = runTest {
        // Given
        val testPosts = listOf(Post(id = "1", title = "Test"))
        ...
        // Then
        val state = viewModel.uiState.value
        assertEquals(testPosts, state.data.posts)
        assertFalse(state.loading)
    }
    ...
    fun `loadPosts handles error`() = runTest {
        ...
        // Then
        assertEquals(error, viewModel.uiState.value.error.get())
    }
}
```

See [state-management.md](./state-management.md#testing-state-management) for complete testing guide.

---

### How do I test repositories?

Use fake data sources for unit testing:
...
class PostsRepositoryTest {
    ...
    @Test
    fun `observePosts emits local data`() = runTest {
        // Given
        val testPosts = listOf(PostEntity(id = "1", title = "Test"))
    ...
    @Test
    fun `syncPosts pushes local changes`() = runTest {
        // Given
...
class ScreenTest {
    ...
    @Test
    fun `clicking post triggers callback`() {
        var clickedPostId: String? = null

        composeTestRule.setContent {
            PostCard(
                post = Post(id = "1", title = "Test Post"),
                onClick = { clickedPostId = it.id }
            )
        }
